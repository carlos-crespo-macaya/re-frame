...
class TransparencyData(BaseModel):
    ...
    techniques_applied: list[str] = Field(
        ...
        description="CBT techniques used in the reframing"
    )
    ...
    confidence: float | None = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Model confidence in the reframing (0-1)"
    )
    stage: str | None = Field(
        None,
        description="Processing stage where any issues occurred"
    )
...
class ReframeResponse(BaseModel):
    ...
    success: bool = Field(
        ...,
        description="Whether the reframing was successful"
    )
    response: str = Field(
        ...,
        description="The reframed perspective or support message"
    )
    ...
    techniques_used: list[str] = Field(
        default_factory=list,
        description="List of CBT techniques applied"
    )
...
class ErrorResponse(BaseModel):
    ...
    error_type: str | None = Field(
        None,
        description="Error classification for client handling"
    )
...
class TechniqueInfo(BaseModel):
    ...
    helpful_for: list[str] = Field(
        ...,
        description="Thought patterns this technique addresses"
    )
...
class TechniquesResponse(BaseModel):
    ...
    note: str = Field(
        ...,
        description="Additional context about the techniques"
    )
...
class SessionInteraction(BaseModel):
    ...
    timestamp: str = Field(
        ...,
        description="ISO timestamp of the interaction"
    )
    thought: str = Field(
        ...,
        description="User's original thought"
    )
    response: str = Field(
        ...,
        description="System's reframed response"
    )
    techniques_used: list[str] = Field(
        default_factory=list,
        description="Techniques applied in this interaction"
    )
...
class SessionHistory(BaseModel):
    ...
    interactions: list[SessionInteraction] = Field(
        default_factory=list,
        description="List of interactions in this session"
    )
...
class SessionHistoryResponse(BaseModel):
    ...
    session: SessionHistory | None = Field(
        None,
        description="Session data if found"
    )
    error: str | None = Field(
        None,
        description="Error message if session not found"
    )
    note: str = Field(
...
class PerformanceMetrics(BaseModel):
    ...
    p95_response_time: float | None = Field(
        None,
        description="95th percentile response time"
    )
    p99_response_time: float | None = Field(
        None,
