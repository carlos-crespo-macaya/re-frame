...
class ADKObservabilityManager:
    ...
    def start_workflow_trace(
        self, workflow_id: str, session_id: str, user_input: str
    ...
    def start_agent_execution(self, agent_name: str, execution_id: str) -> AgentExecutionMetrics:
        """Start tracking an agent execution."""
        metrics = AgentExecutionMetrics(
            agent_name=agent_name, execution_id=execution_id, start_time=datetime.utcnow()
        )
    ...
    def end_agent_execution(
        ...
        if self.debug_mode:
            logger.debug(
                f"Ended agent execution: {metrics.agent_name} ({execution_id}), success: {success}"
            )
    ...
    def get_workflow_trace(self, workflow_id: str) -> WorkflowExecutionTrace | None:
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        for agent_name, metrics_list in self.agent_metrics.items():
            ...
            avg_duration = (
                sum(m.duration_seconds or 0 for m in metrics_list) / len(metrics_list)
                if metrics_list
                else 0
            )
    ...
    def cleanup_old_traces(self, max_age_hours: int = 24):
        ...
        old_workflow_ids = [
            wf_id
            for wf_id, trace in self.execution_traces.items()
        ...
        for agent_name in self.agent_metrics:
            self.agent_metrics[agent_name] = [
                metrics
                for metrics in self.agent_metrics[agent_name]
