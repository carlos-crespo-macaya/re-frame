...
class ThoughtAnalysisInput(BaseModel):
    ...
    context: str | None = Field(
        default=None, description="Additional context about the situation"
    )
...
class CrisisAssessmentInput(BaseModel):
    ...
    urgency_level: str | None = Field(default="standard", description="Assessment urgency level")
...
def create_thought_analysis_tool() -> Tool:
    ...
    def analyze_thought(input_data: ThoughtAnalysisInput) -> ThoughtAnalysisOutput:
        ...
        distortion_patterns = {
            ...
            "emotional_reasoning": [
                r"\b(feel like|feels like|i feel)\b.*\b(so it must be|therefore|means)\b"
            ],
        ...
        avpd_pattern_checks = {
            ...
            "social_avoidance": [
                r"\b(avoid|stay away|don't want to|cancel|skip)\b.*\b(social|people|meeting)\b"
            ],
...
def create_crisis_assessment_tool() -> Tool:
    ...
    def assess_crisis_risk(input_data: CrisisAssessmentInput) -> CrisisAssessmentOutput:
        ...
        if risk_level == "critical":
            ...
            recommended_resources = ["self_care_resources", "mental_health_professional_if_needed"]
...
class CBTTechniqueInput(BaseModel):
    ...
    user_readiness: str = Field(
        default="medium", description="User's readiness for change: low, medium, high"
    )
...
def create_cbt_technique_selector() -> Tool:
    ...
    def select_cbt_techniques(input_data: CBTTechniqueInput) -> CBTTechniqueOutput:
        ...
        if user_readiness == "low":
            ...
            gentle_techniques = ["validation", "self_compassion", "psychoeducation"]
            ...
            secondary_techniques.extend(["behavioral_experiments", "exposure_exercises"])
        ...
        if emotional_intensity > 7:
            approach_notes += (
                "High emotional intensity - prioritize validation and emotional regulation. "
            )
        if "fear_of_criticism" in avpd_patterns:
