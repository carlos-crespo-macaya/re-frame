...
class WorkflowConfiguration(BaseModel):
    ...
    transparency_level: str = Field(default="detailed", regex="^(minimal|standard|detailed|educational)$")
...
class ADKSettings(BaseSettings):
    ...
    # Model settings
    adk_model_provider: str = Field(default="gemini", description="Model provider (gemini, openai, etc.)")
    adk_model_name: str = Field(default="gemini-1.5-flash", description="Model name to use")
    adk_model_temperature: float = Field(default=0.7, ge=0.0, le=2.0, description="Model temperature")
    ...
    adk_enable_observability: bool = Field(default=True, description="Enable observability features")
    ...
    adk_enable_session_persistence: bool = Field(default=True, description="Enable session persistence")
    
    # Safety settings
    adk_enable_crisis_detection: bool = Field(default=True, description="Enable crisis detection")
    adk_crisis_escalation_threshold: str = Field(default="high", description="Crisis escalation threshold")
    adk_content_filtering_enabled: bool = Field(default=True, description="Enable content filtering")
...
class ADKConfigurationManager:
    ...
    def get_workflow_config(self, workflow_name: str = "default") -> Optional[WorkflowConfiguration]:
